...
export async function PATCH(
  ...
  try {
    const result = await db.transaction(async (tx) => {
      ...
      // decision === 'APPROVED'
      const timeOffType = await tx.orm.public.TimeOffType.where({ id: request.timeOffTypeId }).first();
      if (!timeOffType) {
        throw { status: 400, message: 'Time off type no longer exists' };
      }

      // If this leave type doesn't require an allocation, skip the balance check
      // and deduction entirely — just approve the request.
      if (!timeOffType.requiresAllocation) {
        ...
        return { request: updatedRequest, allocation: null };
      }

      // Otherwise, find the matching allocation and deduct
